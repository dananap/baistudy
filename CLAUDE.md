...
- **Vocabulary** is split in two by `Word.owner_user_id`: a curated **catalog** (NULL owner — HSK 1–5, 2496 words, imported from vendored lists) that everyone sees, plus words **owned** by the user who created them. `hanzi` is unique per scope, not globally, so your own 苹果 and the catalog's coexist and `deck.resolve_word` hands each user theirs. Everything goes through `flashcard-backend/deck.py`: `visible_words` for anything offering words to a user, `resolve_word` for hanzi lookups, `require_in_deck` for mutations, `add_to_deck` so an existing word gives you cards rather than a duplicate error.

  Import or re-import the catalog with `cd flashcard-backend && python import_catalog.py --dry-run` (then without the flag, and restart the backend so the segmenter reloads). It is insert-only: it never modifies an existing row, so user-written definitions, examples, notes and images can't be overwritten. `--fill-blanks` additionally populates *empty* fields on catalog rows, which is how example sentences and notes added later reach a database that was imported earlier.

  Catalog content is generated once into the committed JSON rather than per database: `python catalog/build_hsk_catalog.py` rebuilds the word list from the vendored HSK sources, and `python catalog/enrich_catalog.py` adds level-appropriate example sentences and usage notes via the LLM (validated by segmenting each sentence against the vocabulary a learner at that level can actually read).
